...
_quests_cache = {}  # token -> {"data": list, "ts": float}
...
def _clean_hang(token, key):
    try:
        hangs.get(token, {}).pop(key, None)
                    ...
                    while session_id is None or endpoint is None:
                        ...
                        if ev["op"] == 0:
                            if ev["t"] == "VOICE_STATE_UPDATE":
                                d = ev["d"]
                                if d.get("guild_id") == guild_id and d.get("channel_id") == channel_id and (not d.get("user_id") or d.get("user_id") == user_id):
                        ...
                        while True:
                            ...
                            if recv_task.done():
                                try:
                                    ...
                                    if ev.get("s") is not None:
                                        last_seq = ev["s"]
                                    if ev.get("op") == 0 and ev.get("t") == "VOICE_STATE_UPDATE":
                                        d = ev.get("d") or {}
                                        if d.get("user_id") == user_id:
                                            cid = d.get("channel_id")
                                            if not cid or cid != channel_id:
                                                xa_log(token, key, "Đã bị ngắt kết nối hoặc chuyển phòng khỏi kênh voice")
                                                call(stop_event.set)
                                                _clean_hang(token, key)
                                                break
                                    elif ev.get("op") in (7, 9):
                                        xa_log(token, key, f"Gateway yêu cầu reconnect/invalid session (op {ev.get('op')})")
                                        break
                                except Exception:
                                    break
                                recv_task = asyncio.ensure_future(ws.recv())
...
def api_hang():
    ...
    # Dừng kênh voice cũ của token này (Discord chỉ cho phép 1 voice per account)
    for old_key, old_val in list(hangs.get(token, {}).items()):
        if old_key != key:
            try:
                call(old_val["stop"].set)
            except Exception:
                pass
            hangs.get(token, {}).pop(old_key, None)
            (xa_state.get(token) or {}).pop(old_key, None)
...
def api_quests():
    ...
    if quests is None:
        cached = _quests_cache.get(token)
        if cached and (time.time() - cached.get("ts", 0) < 25):
            quests = cached.get("data") or []
        else:
            try:
                ...
                if r.status_code == 200:
                    quests = (r.json() or {}).get("quests") or []
                    _quests_cache[token] = {"data": quests, "ts": time.time()}
                elif cached:
                    quests = cached.get("data") or []
            except Exception:
                if cached:
